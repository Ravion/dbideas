...
public class MySqlSyntaxGenerator implements ISyntaxGenerator {
	...
	public void changeColumn(StringBuilder sb, String tableQualifiedName,
		...
		}else if (type.equals("REAL")||//La size è facoltativa, se c'è ci vuole anche la precisione
			type.equals("DOUBLE")||
			type.equals("FLOAT")
			){
			if(size.length()>0){
		...
		}else if (type.equals("DECIMAL")|| type.equals("NUMERIC")){//Size e precisione obbligatori
			sb.append("("+size+","+decDigits+") ");
		}else if (type.equals("CHAR")||type.equals("VARCHAR")){//Size obbligatoria
			sb.append("("+size+") ");
		}
		if(!acceptNull)
			sb.append(" NOT NULL ");
